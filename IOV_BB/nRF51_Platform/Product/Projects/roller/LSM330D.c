...
void LSM330D(interface_mode interface, uint8_t gAddr, uint8_t xmAddr)
{
    ...
    begin(G_SCALE_250DPS, A_SCALE_2G, G_ODR_95_BW_125, A_ODR_50);
}
...
void calLSM330D(float * gbias, float * abias)
{  
  ...
  nrf_delay_ms(20);                                 // Wait for change to take effect
  gWriteByte(FIFO_CTRL_REG_G, 0x20 | 0x1F);  // Enable gyro FIFO stream mode and set watermark at 32 samples
  nrf_delay_ms(1000);  // nrf_delay_ms 1000 milliseconds to collect FIFO samples
...
void gWriteByte(uint8_t subAddress, uint8_t data)
{
	...
	if (interfaceMode == MODE_I2C)
		I2C_bus_write_byte(gAddress, subAddress, data);
...
void xmWriteByte(uint8_t subAddress, uint8_t data)
{
	...
	if (interfaceMode == MODE_I2C)
		I2C_bus_write_byte(xmAddress, subAddress, data);
...
uint8_t gReadByte(uint8_t subAddress)
{
	...
	// gyro-specific I2C address or SPI CS pin.
	if (interfaceMode == MODE_I2C)
		return I2C_bus_read_byte(gAddress, subAddress);
...
void gReadBytes(uint8_t subAddress, uint8_t * dest, uint8_t count)
{
	...
	// gyro-specific I2C address or SPI CS pin.
	if (interfaceMode == MODE_I2C)
		I2C_bus_read(gAddress, subAddress, dest, count);
...
uint8_t xmReadByte(uint8_t subAddress)
{
	...
	// accelerometer-specific I2C address or SPI CS pin.
	if (interfaceMode == MODE_I2C)
		return I2C_bus_read_byte(xmAddress, subAddress);
...
void xmReadBytes(uint8_t subAddress, uint8_t * dest, uint8_t count)
{
	...
	// accelerometer-specific I2C address or SPI CS pin.
	if (interfaceMode == MODE_I2C)
		I2C_bus_read(xmAddress, subAddress, dest, count);
