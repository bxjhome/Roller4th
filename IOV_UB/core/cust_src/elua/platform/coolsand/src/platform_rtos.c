...
int platform_make_dir(char *pDir, int len)
{
    ...
    pUnicodeDir = IVTBL(malloc)((length+1)*sizeof(WCHAR));
    strtows(pUnicodeDir, pDir);
    
    
    if(IVTBL(make_dir)(pUnicodeDir, 0) == 0)
    {
        IVTBL(free)(pUnicodeDir);
        return 1;
    }
    else
    {
...
int platform_remove_dir(char *pDir, int len)
{
    ...
    if(IVTBL(remove_dir_rec)(pUnicodeDir) == 0)
    {
        IVTBL(free)(pUnicodeDir);
        return 1;
    }
